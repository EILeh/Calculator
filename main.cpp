...
void execute_command(double (*f)(double, double), string param1, string param2)
{
    double p1, p2;

    bool is_numeric = string_to_double(param1, p1) && string_to_double(param2, p2);

    // Checks if is numerical.
    if ( !is_numeric )
    {
        ...
        for ( Command const &comm : COMMANDS )
        {
            ...
            if ( comm.str == command_to_be_executed )
            {
                ...
                if ( comm.parameter_number == pieces.size() - 1 )
                {
                    ...
                    // Do command.
                    execute_command(comm.action, pieces.at(1), pieces.at(2));
